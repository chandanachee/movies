...
const App = () => {
  ...
  const recomMovies = 
    data && data.components.filter(item => {
      return item.type === "movie-list"
    })

  const dropdowns = data && data.components.filter(item => {
    return item.type === "order-select"
  })

 
  useEffect(()=> {
    dispatch(loadDataAction(movieData))
  },[]);
  ...
  const handleChange = (e) => {
	...
	return (
        ...
        <InputLabel htmlFor="age-native-simple">Order By</InputLabel>
        <Select
          native
        ...
        >
          {dropdowns && dropdowns[0] && dropdowns[0].items.map((item, index) => (
            <option key={index} value={item.valueToOrderBy} >{item.label}</option>
          ))}
        </Select>
      </FormControl>
			</div>
